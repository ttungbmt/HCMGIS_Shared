      ...
      root.$children.forEach(component => {
        if (this.componentIsDependency(component)) {
          ...
          component.$watch(attribute, (value) => {
            ...
            if(_.isArray(value)) value = _.map(value, 'value')
    ...
    updateDependencyStatus() {
      ...
      for (let dependency of this.field.dependencies) {
        ...
        if (dependency.hasOwnProperty('value')) {
          if(_.isArray(dependencyValue)){
            let value = _.castArray(dependency.value)
            if(!_.isEmpty(_.intersectionWith(dependencyValue, value))){
              this.dependenciesSatisfied = true;
              return;
            }
          } else {
