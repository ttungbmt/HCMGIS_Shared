    ...
    export default {
        ...
        methods: {
            ...
            async fetchOptions(filters) {
                ...
                if(!_.isEmpty(filters) && _.every(this.filter.dependentOf, attr => !_.isEmpty(filters[attr]))) this.updateFilterOptions()
            },
            updateFilterOptions(){
              const filterIndex = _.findIndex(this.$store.getters[`${this.resourceName}/currentFilters`], {class: this.filterKey})
              if(filterIndex > -1) this.$set(this.$store.state[this.resourceName].filters[filterIndex], 'options', this.availableOptions)
            }
        },
