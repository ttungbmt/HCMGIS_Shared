    ...
    <template v-if="hasValue">
      <div class="flex cursor-pointer">
        <tippy v-bind="tippyOptions" class="index-map">
          ...
          <l-map class="index-map-content" v-bind="mapOptions" style="width: 320px;height: 250px; margin: -5px -9px;"
                 v-if="!loading">
...
import {isEmpty, get, defaultTo, set, transform} from "lodash-es";
...
export default {
  ...
  computed: {
     ...
     */
    hasValue() {
      return !isEmpty(get(this.field, 'value.0.data', []))
    },
    computedLayers() {
  ...
  methods: {
    getMapOptions() {
      return transform(defaultTo(this.field.mapOptions, {}), function(result, value, key) {
        switch (key){
          case 'zoomControl':
            set(result, `options.${key}`, value)
            break;
          case 'bounds':
            result[key] = getBounds(value)
            break;
          default:
            result[key] = value
            break
