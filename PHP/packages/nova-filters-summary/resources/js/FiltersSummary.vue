    ...
    <div class="filters-summary-card" v-if="activeFilters.length">
        ...
        <div class="flex flex-wrap">
            <div class="fsc-filter flex bg-white shadow px-1 py-1 mr-2 mb-2"
                ...
                <div class="ml-2">
                    <div class="remove-filter flex align-items-center justify-center font-bold bg-40 cursor-pointer hover:text-white hover:bg-90" :class="card.stacked ? 'rounded' : 'rounded-full'" @click="del(filter)">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon-x" viewBox="0 0 20 20" fill="currentColor">
        ...
        getActiveFilters (filters = this.$store.getters[`${this.resourceName}/filters`]) {
            this.activeFilters = _.cloneDeep(filters)
                ...
                .map(filter => {
                    ...
                    else if (_.includes(['awesome-nova-dependent-filter'], filter.component)) {
                        filter.summary = filter.options
                            .find(o => this.nin(o.value) === this.nin(filter.currentValue))?.label
                    }
